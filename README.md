...
المتصفح يخزّن هذه الملفات. بعد أي تعديل فيها، ارفع رقم الإصدار في كل الصفحات (`?v=3` → `?v=4`) عشان جوال يامن يحمّل النسخة الجديدة:

```bash
grep -rl "?v=3" index.html lessons | xargs sed -i "s/?v=3/?v=4/g"
```
