...
**المواد الجاهزة:** ACCT 203 الفصل ١ (٤ دروس) · MGT 102 الفصل ١ (٤ دروس) — من كتاب *Contemporary Management* (Jones & George).
...
المتصفح يخزّن هذه الملفات. بعد أي تعديل فيها، ارفع رقم الإصدار في كل الصفحات (`?v=4` → `?v=5`) عشان جوال يامن يحمّل النسخة الجديدة:

```bash
grep -rl "?v=4" index.html lessons | xargs sed -i "s/?v=4/?v=5/g"
```
...
- [ ] دليل مقرر **MGT 102** — أهم مستند ناقص (الكتاب صار موجوداً؛ الفصول ٢–٦ محفوظة نصاً وتُشرح عند الحاجة)
